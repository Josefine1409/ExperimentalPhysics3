...
%             plot(X([channelInterval(1,1):channelInterval(1,2)]),Y([channelInterval(1,1):channelInterval(1,2)]),'*')
            legendString{end+1} = ['t [cm]:' num2str(plateThikness{i}(j))];
        end 
     legend(legendString)
%     figure
...
%         MSE
    ...
    plot(xs,linFun(beta,xs),'-','Linewidth',1.5,'DisplayName',[name{i} ' fit,'])
    
    %     plot(xs,linFun([beta(1),-mu(i)],xs),'--','Linewidth',1.5)
    
    errorbar(x,y,yerr,'.','markersize',10,'DisplayName',[name{i} ' data,'])
...
saveas(fig,['..\figure\Atenuation.png'])
